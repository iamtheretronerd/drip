...
export async function logOutfit(
    ...
    try {
        ...
        const updatePromises = items.map(async (item: any) => {
            ...
            } else if (lastWornDate === today) {
                // If they already logged it today, don't double count stats if we don't want to, 
                // but usually log = action, so we increment times_worn.
